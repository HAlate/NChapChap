...
class NotificationService {
  ...
  void _handleNotification(Map<String, dynamic> notification) {
    ...
    print('[NotificationService] 🔔 Traitement notification type: $type');
    print('[NotificationService] Data: ${notification['data']}');

    if (type == 'incoming_call') {
      // Vérifier que l'appel vient bien du rider (pas un appel que le driver a créé)
      final data = notification['data'] as Map<String, dynamic>?;
      final callerType = data?['caller_type'] as String?;

      if (callerType != 'rider') {
        print(
            '[NotificationService] ⏭️ Notification ignorée: caller_type=$callerType (attendu: rider)');
        return;
      }
      ...
      if (age.inSeconds < 10) {
        ...
        // La marquer comme lue sans afficher le dialog
        _markAsRead(notification['id'] as String);
      }
    }
  }
